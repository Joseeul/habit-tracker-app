...
export default function StreaksScreen() {
  ...
  useEffect(() => {
    if (user) {
      ...
      const completionsChannel = `databases.${DATABASE_ID}.collections.${COMPLETIIONS_COLLECTION_ID}.documents`;
  ...
  const fetchCompletions = async () => {
    try {
      const response = await databases.listDocuments<HabitCompletions>(
        DATABASE_ID,
        COMPLETIIONS_COLLECTION_ID,
        [Query.equal("user_id", user?.$id ?? "")]
      );
  ...
  const getStreakData = (habitId: string): StreakData => {
    ...
    habitCompletions?.forEach((c) => {
      ...
      if (currentSreak > bestStreak) bestStreak = currentSreak;
      streak = currentSreak;
      lastDate = date;
    });
